...
class limition:
    ...
    def updateObs(self,if_test=False):
        ...
        if if_test:
            """测试环境"""
            self.timelog, dic = obs_list[11](self.timelog, self.timeStep)
        else:
            """否则用reset时随机的一个环境"""
    ...
    # def getqNext_sta_dy(self, epsilon, action, q, qBefore, obsCenter, vObs, row0, sigma0, theta):   #eta和epsilon需要外部提供，eta1List为球的斥力列表，eta2List为圆柱体的斥力表 fix:2021.2.9
    #     """
    #     当qBefore为[None, None, None]时，意味着q是航迹的起始点，下一位置不需要做运动学约束，否则进行运动学约束
    #     """
    #     qBefore = np.array(qBefore)
    ...
    def getqNext_sta_dy_change(self, epsilon, action, q, qBefore, obsCenter, vObs, row0, sigma0, theta):   #eta和epsilon需要外部提供，eta1List为球的斥力列表，eta2List为圆柱体的斥力表 fix:2021.2.9
        """
        当qBefore为[None, None, None]时，意味着q是航迹的起始点，下一位置不需要做运动学约束，否则进行运动学约束
        """
        qBefore = np.array(qBefore)
