...
def getReward(flag,apf,qNext):           #计算reward函数
    reward = 0
    if qNext[-1] >= max(apf.maxminhigh):
        reward -= (qNext[-1] - max(apf.maxminhigh)) * 5
    elif qNext[-1] < min(apf.maxminhigh):
        reward -= (min(apf.maxminhigh) - qNext[-1]) * 5
...
def getReward_change(flag, apf, qNext):
    reward = 0
    if qNext[-1] >= max(apf.maxminhigh):
        reward -= (qNext[-1] - max(apf.maxminhigh)) * 10
    elif qNext[-1] < min(apf.maxminhigh):
        reward -= (min(apf.maxminhigh) - qNext[-1]) * 10
    # Reward_col
    if flag[0] == 0:
        # 如果没有发生碰撞，计算与多边体的距离
        if flag[1] == 0:
