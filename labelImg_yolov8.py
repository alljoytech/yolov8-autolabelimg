...
import sys
sys.path.append(os.path.abspath('pytorch_yolov8'))
# 导入 YOLO
from ultralytics import YOLO
sys.path.remove(os.path.abspath('pytorch_yolov8'))

# 添加 YOLOv5 路径并导入
sys.path.append(os.path.abspath('pytorch_yolov5'))
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def yolov8_auto_labeling(self):
        try:
            ...
            def change_result_type_yolov8(boxes, scores, labels):
                result = []
            ...
            weight_path='pytorch_yolov8/ultralytics/weights'
            weight_list=[]
            ...
            if len(weight_list)>0 :
                ...
                "Model weights file(weights file should under 'pytorch_yolov8/ultralytics/weights'):", items, 0, False)
                if not ok:
                    return
                else:
                    weights=os.path.join(weight_path,weights)
            else:
                weights,_ = QFileDialog.getOpenFileName(self,"'pytorch_yolov8/ultralytics/weights' is empty, choose model weights file:")
            ...
            if len(label_list)== 1:
                needed_labels=label_list
            else:
                needed_labels=easygui.multchoicebox(msg="select labels you want auto-labeing?",title="Setect labels",choices=tuple(label_list))
            # set imsize
            # set image size
            imgsz, OK = QInputDialog.getInt(self, 'Integer input dialog', 'input img size (must be multiple of 32):',
                                            value=1920)
            if not OK:
                return
            ...
            progress.setLabelText(u"auto-labeling with yolov8 now,Please wait...")
            ...
            for path, img, im0s, vid_cap in dataset:
                ...
                results = model.predict(img, augment=False)
                # Process detections
                for result in results:
                    ...
                    if boxes_data is not None and len(boxes_data) > 0:
                        ...
                        # Write results
                        for cls_id, conf, xyxy in zip(class_ids, confidences, xyxy_boxes):
                            label_name = names[int(cls_id)]
                            if label_name not in needed_labels:
                                continue
                    ...
                    result = change_result_type_yolov8(boxes, scores, labels)
                    if len(result) > 0:
